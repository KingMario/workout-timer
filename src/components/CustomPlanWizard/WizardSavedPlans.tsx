...
interface WizardSavedPlansProps {
  ...
  onCopyPlan: (plan: SavedPlan) => void;
  onClose: () => void;
}
            ...
            >
              ...
              <div className="ml-2 sm:ml-4 shrink-0">
                <button
                  ...
                  disabled={isActive}
                  aria-label={isActive ? '已载入' : '载入'}
                  className={`p-1.5 sm:px-4 sm:py-1.5 text-sm rounded transition-colors font-medium flex items-center gap-1.5 ${
                    isActive
                      ? 'bg-gray-200 dark:bg-zinc-700 text-gray-400 dark:text-zinc-500 cursor-not-allowed'
                      : 'bg-blue-600 text-white hover:bg-blue-700'
                  }`}
                  title={isActive ? '已载入' : '载入'}
                >
                  <svg
                  ...
                  {!isActive && <span className="sm:hidden">载入</span>}
                </button>
              </div>
            </div>
          );
                    ...
                    {!isEditing && (
                      <>
                        <button
                          ...
                          className="p-1.5 sm:px-3 sm:py-1.5 bg-gray-100 dark:bg-zinc-800 text-gray-600 dark:text-gray-300 text-sm rounded hover:bg-gray-200 dark:hover:bg-zinc-700 transition-colors group flex items-center gap-1.5"
                          title="复制 JSON"
                        >
                          <svg
                        ...
                        <button
                          ...
                          disabled={isActive}
                          aria-label={isActive ? '已载入' : '载入'}
                          className={`p-1.5 sm:px-3 sm:py-1.5 text-sm rounded transition-colors font-medium flex items-center gap-1.5 ${
                            isActive
                              ? 'bg-gray-200 dark:bg-zinc-700 text-gray-400 dark:text-zinc-500 cursor-not-allowed'
                              : 'bg-blue-600 text-white hover:bg-blue-700'
                          }`}
                        ...
                        <button
                          ...
                          aria-label="删除"
                          className="p-1.5 sm:px-3 sm:py-1.5 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 text-sm rounded hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors flex items-center gap-1.5"
                          title="删除"
                        >
                          <svg
                            className="w-4 h-4 sm:hidden"
