...
export default function WorkoutTimer() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && 'mediaSession' in navigator) {
      navigator.mediaSession.setActionHandler('play', () => {
        if (mode === 'workout' && !isRunning) {
          togglePlay();
        }
        if (mode === 'periodic' && !isPeriodicRunning) {
          togglePeriodic();
        }
      });
  ...
  }, [isRunning, isPeriodicRunning, togglePlay, togglePeriodic, mode]);

  useEffect(() => {
    if (typeof window !== 'undefined' && 'mediaSession' in navigator) {
      if (mode === 'workout' && steps[currentIdx]) {
        ...
        const title =
          isBreakActive && activeBreakSteps[activeBreakIdx]
            ? `休息中: ${activeBreakSteps[activeBreakIdx].name} (${breakStepTimeLeft}s)`
            : `下次提醒: ${formatTime(periodicTimeLeft)}`;
        navigator.mediaSession.metadata = new MediaMetadata({
          title,
          artist: '灵动健身 - 间歇模式',
          album: `每 ${periodicInterval} 分钟`,
          artwork: [
            { src: '/vercel.svg', sizes: '512x512', type: 'image/svg+xml' },
          ],
        });
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && 'mediaSession' in navigator) {
      const active = mode === 'workout' ? isRunning : isPeriodicRunning;
      navigator.mediaSession.playbackState = active ? 'playing' : 'paused';
    }
  }, [isRunning, isPeriodicRunning, mode]);
